...
public class PartRepository extends UnicastRemoteObject implements PartInterface{
    ...
    @Override
    public Map<Part,Integer> allSubParts(Part part) throws RemoteException {
    ...
    @Override
    public boolean addSubPart(Part part, Part sub, Integer qnt) throws RemoteException {
    	
    	Predicate<Part> predicate = x -> x.getId().equals(part.getId());
        Part insert = partList.stream().filter(predicate).findFirst().get();
    	
    	insert.addSubPart(sub,qnt);    	
    	return true;
    }
